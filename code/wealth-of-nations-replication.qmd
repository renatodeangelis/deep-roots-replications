---
title: "Replication of Comin, Easterly, and Gong (2010)"
format: pdf
execute:
  echo: true
  warning: false
...
knitr::opts_chunk$set(root.dir = "Downloads/Replication_Wealth_of_Nations")
```
...
```{r}
wb_data = read_csv("world-regions-according-to-the-world-bank.csv")
setwd("~/Downloads/Replication_Wealth_of_Nations")
macro_data = read_dta("primitive_aejmacro.dta")
```
...
The following code replicates Column 1 of Table 8A (impact of technology
adoption in 1000 BC on log per capita income in 2002). The first equation follows 
Comin et al's regression, while the second follows Kelly and introduces controls
for World Bank regions. In both cases, standard errors are clustered.
...
This chunk replicates Column 3 of Table 8A (impact of technology
adoption in 1500 AD on log per capita income in 2002).
...
In both cases, adding the World Bank region dummies reduces the effect size
significantly. In the case of technology adoption in 1000 BC, the effect
reverses direction. In the original paper, the authors control for continental
variation in Table 10. I tried to replicate these equations, but got different
effects. I'm not sure how to account for these differences. 
...
The following code adds the World Bank region dummies to the migration-adjusted
technology measure regressions.
